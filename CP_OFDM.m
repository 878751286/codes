...
len_data = T_data*fs;%信息序列的点数也是一个OFDM符号的点数长度
N_cp = 128;%循环前缀的长度
...
%% ==================基带调制===========================
% 加窗,升余弦滚降窗
data_win= conv(hrc,data); %将1024bit个信息序列加窗
data_win = data_win(hrc_order/2+1:end-hrc_order/2);

% 一个OFDM符号
t1 = 0:Ts:T_data-1/fs; % 一个OFDM符号的时间轴，按照点数来的
...
figure(01);
subplot(2,1,1);
plot(t1,xt); %绘制通带信号波形，s为单位
title('时域波形');xlabel('\bf Time(s)');ylabel('\bf Amplitude');
...
subplot(2,1,2);
plot(freq_axis,abs(Xt)/max(abs(Xt)));
title('归一化频谱分析');xlabel('\bf Freq(Hz)');ylabel('\bf Amplitude(V)');

%%  ==================  IFFT方法  ================================
ifft_ofdm = ifft(data_win,len_data)/N_data;% IFFT变换,做多少点的IFFT变换呢？
ifft_cp = ifft_ofdm(end-N_cp/N_data*length(ifft_ofdm)+1:end);% 插入循环前缀
...
title('IFFT-时域波形');xlabel('\bf Time(s)');ylabel('\bf Amplitude');
% 进行频谱分析
ifft_Nfft = 2^nextpow2(length(ifft_pb_xt));
ifft_Xt = fft(ifft_pb_xt,ifft_Nfft)/length(ifft_pb_xt);
ifft_freq_axis =(0:ifft_Nfft/2-1)/ifft_Nfft*fs ;%频率轴
subplot(2,1,2);plot(ifft_freq_axis,abs(ifft_Xt(1:ifft_Nfft/2))/max(abs(ifft_Xt))); %绘制归一化频谱
title('IFFT-归一化频谱分析');xlabel('\bf Freq(Hz)');ylabel('\bf Amplitude(V)');
